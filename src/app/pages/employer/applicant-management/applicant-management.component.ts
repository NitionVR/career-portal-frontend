...
import { ApplicantManagementService, Applicant } from './applicant-management.service';
...
export class ApplicantManagementComponent implements OnInit {
  ...
  loadApplications(): void {
    ...
    this.applicantService.getApplicants(params).subscribe({
      next: (data: Applicant[]) => {
  ...
  matchesSearchTerm(app: Applicant, term: string): boolean {
  ...
  getAIScore(application: Applicant): number {
    return application.aiMatchScore;
  }
  ...
  getStatusColor(status: string): string {
    // Use the custom status-chip class and specific status classes defined in CSS
